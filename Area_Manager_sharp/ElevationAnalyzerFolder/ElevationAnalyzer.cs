...
namespace Area_Manager_sharp.ElevationAnalyzer
{
	...
	internal class ElevationAnalyzer
	{
			...
			while (attempt < _maxAttempts)
			{
				...
				try
				{
					...
					if (response.IsSuccessStatusCode)
					{
						...
						Console.WriteLine($"Высота точки {latitude}, {longitude}: {result}");
						break; // Успешный ответ, выходим из цикла
					}
					else
					{
						logger.Warn($"Request failed: {response.StatusCode}. Retrying in 5 seconds...");
						Console.WriteLine($"Request failed: {response.StatusCode}. Retrying in 5 seconds...");
						attempt++;
		...
		public async Task<PointsPack> findArea(Сoordinate coordinate, double initialHeight = 200, double distance = 200, bool equalOption = false, bool useInflux = false)
		{
			...
			Queue<Сoordinate> pointsToDebug = new Queue<Сoordinate>();
			...
			while (pointsToCheck.Count != 0)
			{
				...
				if (comparison(currentElevation, checkPoint._height))
				{
					...
					pointsToDebug.Enqueue(checkPoint._сoordinate);
				...
				if (_debug && pointsToDebug.Count >= 10)
				{
					Сoordinate debugPoint = pointsToDebug.Dequeue();
